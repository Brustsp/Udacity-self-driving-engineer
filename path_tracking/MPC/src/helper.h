...
vector< vector<int> > get_next_lane_and_vehicle(vector<vector<double> > const& sensor_fusion, int car_d, int car_s){
  ...
  // Find the next lanes' nearest cars at behind and in front.
...
int get_safety_next_lane(vector<vector<double>> const& sensor_fusion, vector<vector<int>> next_lanes, double car_s, double car_speed)
{
  ...
  for(int i = 0; i < next_lanes.size(); i++){
      ...
      if(next_behind_car_id == -1)  next_behind_s = -1;
      else{
      ...
      if(next_front_car_id == -1){ 
        ...
        if(next_behind_car_id != -1 && car_s - next_behind_s < 10){
          ...
          // cout << "INFO: lane " << best_next_lane_id << " is safe to change.." << endl;
          break;
        }
      ...
      if(next_front_s >= car_s + 30 && next_behind_s <= car_s - 8){
          ...
          // cout << "lane " << best_next_lane_id << " is safe to change." << endl;
      } else {
        // string str; 
        // stringstream stream; 
        // stream << next_lanes[i][0];
        // stream >> str;
        // string reason = "lane " + str + " is NOT safe to change. Because";
        string reason = "lane " + std::to_string(next_lanes[i][0]) + " is NOT safe to change. Because";
  ...
  if(best_next_lane_id == -1)
    std::cout << "Warning: No safe lane to change." << std::endl;
  else
    std::cout << "INFO: lane " << best_next_lane_id << " is safe to change." << std::endl;
